...
export function Nav({ connected: connectedProp }: { connected?: boolean }) {
  ...
  useEffect(() => {
    if (connectedProp !== undefined) return;
      ...
      <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
              ...
              return (
                <Link
                  ...
                  className={`relative px-3.5 py-1.5 rounded-lg text-sm font-medium transition-all duration-300 flex items-center gap-2 ${
                    isActive
                      ? 'text-mesh-accent-light bg-mesh-accent/10 tab-active'
                      : 'text-mesh-muted hover:text-gray-300 hover:bg-white/[0.03]'
                  }`}
                >
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-mesh-card/50 border border-mesh-border/50">
            ...
            <span className="text-[11px] text-mesh-muted font-medium">
          ...
          <a
            ...
            className="flex items-center gap-1.5 text-[11px] text-mesh-muted hover:text-mesh-accent-light transition-colors duration-300"
          >
