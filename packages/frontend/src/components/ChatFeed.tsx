...
const MSG_TYPE_CONFIG: Record<string, { bg: string; text: string; icon: string; label: string }> = {
  ...
  fallback: { bg: 'bg-yellow-500/10', text: 'text-yellow-400', label: 'Fallback', icon: 'M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z' },
  ...
  'vote-abstain': { bg: 'bg-gray-500/10', text: 'text-gray-400', label: 'Abstain', icon: 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636' },
...
function formatPayload(type: string, payload: unknown): string {
  ...
  switch (type) {
    ...
    case 'fallback':
      return `Deep analysis unavailable. Proceeding with static scan results for ${p.specialty}.`;
    case 'finding': {
    ...
    case 'vote-abstain':
      return `Skipped voting on this finding. ${p.reason || 'Compute unavailable.'}`;
    case 'debate':
